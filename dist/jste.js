...
var jste = function jste(selector) {
  var _this = this;
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var context = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : document;
  var self = context.querySelector(selector);
  ...
  var defaultLinkTypes = ["Web Address", "E-mail Address", "Picture URL"];
  var vars = _extends({
    // options
    'status': true,
    'css': "jste",
  ...
  function selected2link() {
    if (toolbar.dataset.sourceOpened === undefined || toolbar.dataset.sourceOpened === 'false') {
      ...
      linkAreaSwitch(true);
      if (selectedTag) {
        var thisTagName = selectedTag.tagName.toLowerCase();
  ...
  function linkRecord(thisSelection) {
    ...
    if (linktypes.dataset.linktype !== "2") selectionSet("createlink", linkinput.value); // insert link url of link-input to the selected node
    // if selected to link-type of picture
      ...
      editor.querySelectorAll("img").forEach(function (el) {
        var emptyPrevLinks = _prev(el, "a");
        var emptyNextLinks = _next(el, "a");
  ...
  function linkInputSet(thisHrefLink) {
    var currentType = linktypes.dataset.linktype;

    // if selected type of e-mail
    if (currentType === "1" && (linkinput.value === "https://" || linkinput.matches('[value^="https://"]') || !linkinput.matches('[value^="mailto"]'))) linkinput.value = "mailto:";else if (currentType !== "1" && !linkinput.matches('[value^="https://"]')) linkinput.value = "https://";else linkinput.value = thisHrefLink;
  }
  ...
  function selected2style(styleCommand) {
    if (toolbar.dataset.sourceOpened === undefined || toolbar.dataset.sourceOpened === 'false') {
      ...
      styleField.querySelectorAll("a").forEach(function (el) {
        ...
        _addListener(el, 'click', function () {
          var styleValue = this.getAttribute(vars.css + "-styleval"); // the property of style value to be added
  ...
  function styleFieldSwitch(styleField, status) {
    ...
    if (status) {
      ...
      for (var _si = 0; _si < allData.length; _si++) {
        if (mainData["d"] !== allData[_si]["d"]) {
          toolbar.dataset[allData[_si]["d"]] = false;
          _slideUp(allData[_si]["f"], 1000);
        }
      }
    }
    // hide all style-fields
      ...
      for (var _si2 = 0; _si2 < allData.length; _si2++) {
        toolbar.dataset[allData[_si2]["d"]] = false;
        _slideUp(allData[_si2]["f"], 100);
      }
    }
  }
  ...
  function refuseStyle(refStyle) {
    ...
    if (selectedTag && selectedTag.matches("[style]") && selectedTag.style[refStyle] !== "") {
      var refValue = selectedTag.style[refStyle]; // first get key of unwanted style

      selectedTag.style[refStyle] = ""; // clear unwanted style

      var cleanStyle = selectedTag.getAttribute("style"); // cleaned style
  ...
  function selected2format() {
    ...
    formatbar.querySelectorAll("a").forEach(function (el) {
      _addListener(el, 'click', function () {
        ...
        var formatValue = this.getAttribute(vars.css + "-formatval"); // the type of format value
  ...
  function formatLabelView(str) {
    var formatLabel = formatbar.closest("." + vars.css + "_tool").querySelector("." + vars.css + "_tool_label").querySelector("." + vars.css + "_tool_text");
  ...
  function extractToText(strings) {
    ...
    htmlPattern = [/\<span(|\s+.*?)><span(|\s+.*?)>(.*?)<\/span><\/span>/gim,
    // trim nested spans
    /<(\w*[^p])\s*[^\/>]*>\s*<\/\1>/gim,
    // remove empty or white-spaces tags (ignore paragraphs (<p>) and breaks (<br>))
    /\<div(|\s+.*?)>(.*?)\<\/div>/gim,
    // convert div to p
    /\<strong(|\s+.*?)>(.*?)\<\/strong>/gim,
    // convert strong to b
    ...
    if (!vars.br) {
      ...
      for (var _i = 0; _i < htmlPattern.length; _i++) {
        htmlContent = htmlContent.replace(htmlPattern[_i], htmlReplace[_i]);
      }
    }
  ...
  function postToSource() {
    ...
    var sourceStrings = editor.textContent === "" && editor.innerHTML.length < 12 ? "" : editor.innerHTML;
  ...
  */
    ...
    for (var iLstId = 0, iElId = 0, oEvtListeners = oListeners[oEvent.type]; iElId < oEvtListeners.aEls.length; iElId++) {
      if (oEvtListeners.aEls[iElId] === element) {
        for (iLstId; iLstId < oEvtListeners.aEvts[iElId].length; iLstId++) {
          oEvtListeners.aEvts[iElId][iLstId].call(element, oEvent);
        }
        break;
      }
    }
  ...
  function _addListener(element, sEventType, fListener) {
    if (oListeners.hasOwnProperty(sEventType)) {
      ...
      var nElIdx = -1;
      for (var iElId = 0; iElId < oEvtListeners.aEls.length; iElId++) {
        if (oEvtListeners.aEls[iElId] === element) {
          nElIdx = iElId;
          break;
        }
  ...
  var jste = function jste() {
    if (!self) {
      return;
    }
    if (!self.dataset.jste || self.dataset.jste === "undefined") self.dataset.jste = 'true';else self.dataset.jste = 'false';
    ...
    if (!vars.status || self.dataset.jste.toLowerCase() === "false") {
      ...
      if (self.closest("." + vars.css).length > 0) {
        var editorValue = self.closest("." + vars.css).querySelector("." + vars.css + "_editor").innerHTML;
        ...
        var _thisElementTag = self.matches("[data-origin]") && self.getAttribute("data-origin") !== "" ? self.getAttribute("data-origin") : "textarea";

        // the contents of this element
        var createValue = '>' + editorValue;

        // if this element is input or option
        if (_thisElementTag === "input" || _thisElementTag === "option") {
        ...
        var thisClone = self.cloneNode();
        self.dataset.jste = 'false';
        self.closest("." + vars.css).insertAdjacentHTML('beforeend', thisClone).remove();
        thisClone.outerHTML = '<' + _thisElementTag + thisElementAttrs + createValue + '</' + _thisElementTag + '>';
      }
      return;
    }

    // element will convert to the jste editor
    _this.thisElement = self;

    // tag name of the element
    var thisElementTag = self.tagName.toLowerCase();

    // tag name of origin
    self.setAttribute("data-origin", thisElementTag);

    // contents of the element
    var thisElementVal = self.matches("[value]") || thisElementTag === "textarea" ? self.value : self.innerHTML;
    ...
    // jste
    _this.JSTEContainer = _next(self, '.' + vars.css);
    ...
    _this.toolbar = JSTEContainer.querySelector('.' + vars.css + "_toolbar"); // the toolbar variable
    _this.linkform = JSTEContainer.querySelector('.' + vars.css + "_linkform"); // the link-form-area in the toolbar variable
    _this.editor = JSTEContainer.querySelector('.' + vars.css + "_editor"); // the text-field of jste editor
    _this.emphasize = vars.css + "_tool_depressed"; // highlight style of the toolbar buttons
    ...
    _this.linktypeselect = linkform.querySelector("." + vars.css + "_linktypeselect"); // the tool of link-type-selector
    // let linkinput = linkform.querySelector("." + vars.css + "_linkinput"); // the input of insertion link
    _this.linkinput = linkform.querySelector("." + vars.css + "_linkinput"); // the input of insertion link
    // let linkbutton = linkform.querySelector("." + vars.css + "_linkbutton"); // the button of insertion link
    _this.linkbutton = linkform.querySelector("." + vars.css + "_linkbutton"); // the button of insertion link
    ...
    _this.linktypes = linktypeselect.querySelector("." + vars.css + "_linktypes"); // the select box of link types
    _this.linktypeview = linktypeselect.querySelector("." + vars.css + "_linktypeview"); // the link type preview
    _this.setdatalink = vars.css + "-setlink"; // the selected text add to mark as "link will be added"

    // create to the source-area
    editor.insertAdjacentHTML('afterend', '<div class="' + vars.css + '_source ' + vars.css + '_hiddenField"></div>');
    var sourceField = JSTEContainer.querySelector("." + vars.css + "_source"); // the source-area variable
    ...
    if (thisElementTag !== "textarea") {
      ...
      var _thisElementAttrs = "";
      Array.prototype.forEach.call(thisElement.attributes, function () {
        if (this.nodeName !== "type" && this.nodeName !== "value") _thisElementAttrs = _thisElementAttrs + " " + this.nodeName + '="' + this.nodeValue + '"';
      });

      // convert the element to textarea
      thisElement.outerHTML = '<textarea ' + _thisElementAttrs + '>' + thisElementVal + '</textarea>';
    ...
    for (var n = 0; n < buttons.length; n++) {
      ...
      if (vars[buttons[n].name]) {
        ...
        var btnTool = toolbar.querySelector('.' + vars.css + '_tool[data-tool="' + n + '"]');
        ...
        // font-size-selector field
          ...
          // add font-sizes to font-size-selector
          for (var _f = 0; _f < vars.fsizes.length; _f++) {
            toolbar.querySelector("." + vars.css + "_fontsizes").insertAdjacentHTML('beforeend', '<a ' + vars.css + '-styleval="' + vars.fsizes[_f] + '" class="' + vars.css + '_fontsize' + '" style="font-size:' + vars.fsizes[_f] + vars.funit + '" role="menuitem" unselectable>Abcdefgh...</a>');
          }
        }
    ...
    for (var _n = 0; _n < 3; _n++) {
      linktypes.insertAdjacentHTML('beforeend', '<a ' + vars.css + '-linktype="' + _n + '" unselectable>' + vars.linktypes[_n] + '</a>');
    ...
    var prefixCss = "";
    if (/msie/.test(thisBrowser))
      // ie
      prefixCss = '-ms-';else if (/chrome/.test(thisBrowser) || /safari/.test(thisBrowser) || /yandex/.test(thisBrowser))
      // webkit group (safari, chrome, yandex)
      prefixCss = '-webkit-';else if (/mozilla/.test(thisBrowser))
      // firefox
      prefixCss = '-moz-';else if (/opera/.test(thisBrowser))
      // opera
      prefixCss = '-o-';else if (/konqueror/.test(thisBrowser))
      // konqueror
    ...
    var toolbutton = toolbar.querySelectorAll("." + vars.css + "_tool");

    // format menu
    _this.formatbar = toolbar.querySelector("." + vars.css + "_formats");

    // font-size filed
    _this.fsizebar = toolbar.querySelector("." + vars.css + "_fontsizes");

    // color palette
    _this.cpalette = toolbar.querySelector("." + vars.css + "_cpalette");
    ...
    toolbutton.forEach(function (el) {
        ...
        // if other buttons is clicked
          ...
          if (toolbar.dataset.sourceOpened === undefined || toolbar.dataset.sourceOpened === 'false') {
            ...
            // if the style buttons are clicked
              ...
              if (this.dataset.command === 'fSize' && !e.target.classList.contains(vars.css + "_fontsize") ||
      ...
      _addListener(el, 'mouseover', function (e) {
        if (vars.title && this.dataset.title !== "" && (e.target.classList.contains(vars.css + "_tool") || e.target.classList.contains(vars.css + "_tool_icon"))) {
          ...
          var thisTitle = document.querySelector('.' + vars.css + '_title');
    ...
    _addListener(editor, 'keydown', function (e) {
      ...
      if (e.ctrlKey) {
        ...
        for (var _n2 = 0; _n2 < buttons.length; _n2++) {
          // if these settings of this button is activated (is it true)
          // if the keyed button with ctrl is same of hotkey of this button
          if (vars[buttons[_n2].name] && e.keyCode === buttons[_n2].key.charCodeAt(0)) {
            if (buttons[_n2].command !== '' && buttons[_n2].command !== 'linkcreator') selectionSet(buttons[_n2].command, null);else if (buttons[_n2].command === 'linkcreator') selected2link();
            return false;
          }
        }
      }
