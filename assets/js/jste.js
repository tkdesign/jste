...
const jste = function (selector, options = {}, context = document) {
	...
	// methods
	// $.fn.jsteVal = function(value){
	// this.closest("."+vars.css).find("."+vars.css+"_editor").innerHTML=value;
	// }
	...
	return {
		init() {
			if (!self.dataset.jste || self.dataset.jste == null || self.dataset.jste == "undefined")
			...
			if (!vars.status || (self.dataset.jste.toLowerCase() == "false")) {
				...
				if (self.closest("." + vars.css).length > 0) {
					...
					self.dataset.jste = false; self.closest("." + vars.css).insertAdjacentHTML('beforeend', thisClone).remove();
					thisClone.outerHTML = '<' + thisElementTag + thisElementAttrs + createValue + '</' + thisElementTag + '>';
				}
				return;
			}

			// element will converted to the jste editor
			...
			let toolbar = JSTEContainer.querySelector('.' + vars.css + "_toolbar"); // the toolbar variable
			let linkform = JSTEContainer.querySelector('.' + vars.css + "_linkform"); // the link-form-area in the toolbar variable
			let editor = JSTEContainer.querySelector('.' + vars.css + "_editor"); // the text-field of jste editor
			let emphasize = vars.css + "_tool_depressed"; // highlight style of the toolbar buttons
			...
			let linktypeselect = linkform.querySelector("." + vars.css + "_linktypeselect"); // the tool of link-type-selector
			...
			let linktypes = linktypeselect.querySelector("." + vars.css + "_linktypes"); // the select box of link types
			let linktypeview = linktypeselect.querySelector("." + vars.css + "_linktypeview"); // the link type preview
			let setdatalink = vars.css + "-setlink"; // the selected text add to mark as "link will be added"
			...
			editor.setAttribute("contenteditable", "true"); editor.innerHTML = thisElementVal;
			...
			for (let n = 0; n < buttons.length; n++) {
				...
				if (vars[buttons[n].name]) {
					...
					if (buttons[n].name == "format" && Array.isArray(vars.formats)) {
						...
						toolbar.querySelectorAll("." + vars.css + "_formats").forEach(function (el) { el.dataset.status = false; });
					}
			...
			if (vars.placeholder && vars.placeholder != "") {
				...
				placeHolder.addListener('click', function () { editor.focus(); });
			}
			...
			JSTEContainer.querySelectorAll("[unselectable]").forEach(function (el) {
				...
				["selectstart", "mousedown"].forEach(function (e) { el.removeListener(e); });
			});
			...
			function affectStyleAround(element, style) {
				...
				if (selectedTag && style == false) {
					...
					if (selectedTag.matches("[style]"))
						selectedTag.querySelectorAll("*").forEach(function (el) { el.setAttribute("style", selectedTag.getAttribute("style")); });
				}
				// (for replacement with html changing method)
					...
					if (element.matches('[style*="' + styleKey[0] + '"]'))
						element.querySelectorAll("*").forEach(function (el) { el.style[styleKey[0]] = styleKey[1]; });
			...
			function selected2link() {
				if (toolbar.dataset.sourceOpened === undefined || toolbar.dataset.sourceOpened == 'false') {
					let selectedTag = getSelectedNode(); // the selected node
					let thisHrefLink = "http://"; // default the input value of the link-form-field
					...
					if (selectedTag) {
						...
						linkinput.value = thisHrefLink; linkinput.focus();
			...
			function linkAreaSwitch(status) {
				...
				JSTEContainer.querySelectorAll("[" + setdatalink + "][href]").forEach(function (el) { el.removeAttribute(setdatalink); });
			...
			function linkInputSet(thisHrefLink) {
				...
				if (currentType == "1" && (linkinput.value == "http://" || linkinput.matches('[value^="http://"]') || !linkinput.matches('[value^="mailto"]')))
					linkinput.value = "mailto:";
				else if (currentType != "1" && !linkinput.matches('[value^="http://"]'))
			...
			function selected2style(styleCommand) {
				if (toolbar.dataset.sourceOpened === undefined || toolbar.dataset.sourceOpened == 'false') {
					...
					styleField.querySelectorAll("a").forEach(function (el) {
						el.removeAllListeners('click'); el.addListener('click', function () {
							...
							document.querySelectorAll('.' + vars.css + '_title').forEach(function (el) { el.remove(); });
			...
			function clearSetElement(elem) {
				JSTEContainer.querySelectorAll(elem).forEach(function (el) {
					el.insertAdjacentHTML('beforebegin', el.innerHTML); el.remove();
				});
			}
			...
			function formatFieldSwitch(status) {
				...
				if (thisStatus || !status) { formatbar.dataset.status = false; formatbar.slideUp(200); }
				else { formatbar.dataset.status = true; formatbar.slideDown(200); }
			}
			...
			function buttonEmphasize(e) {
				for (let n = 0; n < buttons.length; n++) {
					if (vars[buttons[n].name] && buttons[n].emphasis && buttons[n].tag != '')
						detectElement(buttons[n].tag) ? toolbar.querySelectorAll('.' + vars.css + '_tool_' + buttons[n].cls).forEach(function (el) { el.classList.add(emphasize); }) : document.querySelector('.' + vars.css + '_tool_' + buttons[n].cls).classList.remove(emphasize);
				}
				// showing text format
			...
			toolbutton.forEach(function (el) {
				...
				el.addListener('click', function (e) {
					...
					// if other buttons is clicked
						...
						// hide the source field and display the text field
							...
							toolbar.querySelectorAll("." + vars.css + "_tool").forEach(function (el) { el.classList.remove(vars.css + "_hiddenField"); });
				...
				el.addListener('mouseover', function (e) {
					if (vars.title && this.dataset.title != "" && (e.target.classList.contains(vars.css + "_tool") || e.target.classList.contains(vars.css + "_tool_icon"))) {
						...
						setTimeout(function () { thisTitle.style.top = thisAlignY; thisTitle.style.left = thisAlignX; thisTitle.fadeIn(200); }, 400);
					}
				});
			...
			['mouseup', 'keyup'].forEach(function (event) { editor.addListener(event, buttonEmphasize); });
			...
			editor.addListener('focusout', function () {
				...
				toolbutton.forEach(function (el) { el.classList.remove(emphasize); });
